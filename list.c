...
// FUNCTION #1: Create a new list with "e" as default item.
list *newList(item e) {
...
// FUNCTIONS #3: Current -> first of the list (next).
// FUNCTION #4: Current -> last of the list (back).
void first(list *xs) {
...
// FUNCTION #5: no item = true, else false (none).
bool none(list *xs) {
...
bool after(list *xs) {
  ...
  else xs -> current = xs -> current -> next; 
  return true;
}
...
bool before(list *xs) {
  if (xs -> current  == xs -> none) return false;
  else xs -> current = xs -> current -> back;
  return true;
}

// FUNCTION #8: Get the current item.
item get(list *xs) {
  if (xs -> current == xs -> none) return -1;
  else return xs -> current -> x;
}

// FUNCTION #9: Set the current item + return true, 
...
void insertAfter(list *xs, item x) {
  ...
  *new = (node) {xs -> current, x, xs -> current -> next};
  xs -> current -> next = new;
  new -> next -> back = new;
  xs -> current = new;
...
void insertBefore(list *xs, item x) {
  ...
  *new = (node) {xs -> current -> back, x, xs -> current};
  xs -> current -> back = new;
  new -> back -> next = new;
  xs -> current = new;
} 
...
bool deleteToAfter(list *xs) {
  if (xs -> current != xs -> none) {
    ...
    curr -> next -> back = xs-> current -> back;
    curr -> back -> next = xs-> current -> next; 
...
// preserving the sequence of all list items.
void reform(list *xs) {
  item new = xs -> current -> x;
  while(xs -> current -> back != xs -> none) {
    if (xs -> current == xs -> none) return;
    else {
      // deletes the item in before the selected node and insert them back
      // until no item is in front of the selected node
      deleteToBefore(xs);
      insertBefore(xs, new);
    }
  }
